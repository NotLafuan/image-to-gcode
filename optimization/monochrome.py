...
from utils import shortest_index


def post_process(image: cv2.Mat, threshold: float = 0.5) -> cv2.Mat:
    # image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    # image = cv2.threshold(image, 255 * threshold, 255, cv2.THRESH_BINARY)[1]
    image = cv2.Canny(image, 100/1.86, 200/1.86)
...
def sort_points(points: list[list[int]]) -> list[list[int]]:
    ...
    bar.update()
    while len(points):
        # points = list[list[x, y, surround]]
        new_coord = new_points[-1][-1][:2]
        index = shortest_index(new_coord, points)
        if math.dist(new_coord, points[index][:2]) < 3:
            new_points[-1].append(points.pop(index))
        else:
            new_points.append([points.pop(index)])
        bar.update()
    return new_points
...
if __name__ == '__main__':
    ...
    for point in points:
        point_image[point[0], point[1]] = [0,0,0]
    cv2.imwrite('out.png', point_image)
    ...
    gcode = create_gcode(points=points, image=image, scale=0.1)
